...
function solve_sequential!(prob::IDAPBCProblem, paramvec, data, qdesired; batchsize=64, η=0.001, maxiters=1000)
    ℓ1  = loss_massd
    ∂ℓ1 = ∂loss_massd(prob)
    ℓ2(q,θ) = batchsize*loss_ped(prob,q,θ) + abs2(prob.hamd.potential(qdesired, getindex(θ, prob.ps_index[:potential]))[1]) + (norm(q) < 0.25 ? abs2(prob.hamd.potential(q, getindex(θ, prob.ps_index[:potential]))[1]) : zero(eltype(q)))
